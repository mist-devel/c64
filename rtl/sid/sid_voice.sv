...
localparam signed [21:0] VOICE_DC_6581    = 22'sd212160; // = 22'('h340*'hff)
...
always @(posedge clock) begin
	...
	if (reset) begin
		...
		noise      <= 0;
		osc_edge   <= 0;
		lfsr_noise <= 23'h7FFFFF;  // all-ones
		noise_age  <= 0;
	end
	else begin
		if(ce_1m) begin
			...
			if (~clk) begin
				if (noise_age >= (mode ? NOISE_TTL_8580 : NOISE_TTL_6581)) noise <= 12'hFFF;  // all-ones
