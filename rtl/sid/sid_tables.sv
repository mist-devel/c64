...
function [9:0] tanh_x_mirror(wire signed [10:0] x);
    tanh_x_mirror = (x < 0 ? -x : x) & 10'h3FF;  // 10-bit truncate of abs(x[10:0])
endfunction

function signed [10:0] tanh_x_clamp(wire signed [12:0] x);
    tanh_x_clamp = (x < -1023) ? -11'sd1023 :
                   (x >  1023) ?  11'sd1023 :
                   x[10:0];  // 11-bit truncate of 13-bit signed
endfunction
...
wire signed [15:0] f6581_adj_y0 = 16'sd10133;  // = 16'(9883+250)
wire [10:0] fc_x = tanh_x_clamp($signed({2'b00, fc_6581}) - $signed(Fc_offset));  // zero-extend fc_6581 to 13b
